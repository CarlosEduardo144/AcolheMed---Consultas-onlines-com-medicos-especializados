...
export class ConsultasPage implements OnInit {
  ...
  carregarConsultas() {
    ...
    const tipo = this.loginService.getTipoUsuario(this.usuario) as 'paciente' | 'medico';
  ...
  get listaBusca(): ConsultaModel[] {
    ...
    return base.filter(c =>
      this.nomeMedico(c).toLowerCase().includes(q) ||
  ...
  nomeExibido(c: ConsultaModel): string {
    return this.loginService.getTipoUsuario(this.usuario) === 'medico' ? c.paciente.nome : this.nomeMedico(c);
  }

  private nomeMedico(c: ConsultaModel): string {
    return c.medico.usuario?.nome ?? '';
  }
