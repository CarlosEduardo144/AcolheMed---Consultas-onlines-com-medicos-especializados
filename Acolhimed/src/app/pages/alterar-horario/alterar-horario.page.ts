...
export class AlterarHorarioPage implements OnInit {
  ...
  ngOnInit() {
    ...
    if (this.loginService.getTipoUsuario(usuarioLogado) !== 'medico') {
      this.navController.navigateBack('/perfil');
      return;
    }

    const usuarioSalvo = this.loginService.getUsuario() as MedicoModel;
  ...
  async salvar() {
    ...
    const usuarioSalvo = this.loginService.getUsuario();

    if (!usuarioSalvo || this.loginService.getTipoUsuario(usuarioSalvo) !== 'medico') {
    ...
    this.usuarioService.salvar(medicoAtualizado).subscribe({
      next: () => {
        this.loginService.setUsuario(medicoAtualizado);
        this.usuario = medicoAtualizado;
        this.showToast('Horarios salvos com sucesso!', 'success');
      },
      error: (erro) => {
        this.showToast(erro.error?.message ?? 'Nao foi possivel salvar os horarios.', 'danger');
      },
    });
  }
